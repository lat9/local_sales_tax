                ...
                "INSERT INTO " . TABLE_LOCAL_SALES_TAXES . "
                 ...
                 values
                    (
                       ...
                       '" . zen_db_input($_POST['tax_shipping']) . "',
                ...
                "UPDATE " . TABLE_LOCAL_SALES_TAXES . "
                    SET zone_id = " . (int)$_POST['zone_id'] . ",
                        ...
                        local_datamatch = '" . zen_db_input($_POST['tax_datamatch']) . "',
                            ...
                            local_tax_shipping = '" . zen_db_input($_POST['tax_shipping']) . "',
...
$heading = [];
$contents = [];
$link_params = 'page=' . $_GET['page'] . '&stID=' . $stInfo->local_tax_id;
switch ($action) {
    case 'new':
        $heading[] = ['text' => '<b>' . TEXT_INFO_HEADING_NEW_LOCAL_SALES_TAX . '</b>'];

        $contents = ['form' => zen_draw_form('local_sales_tax', FILENAME_LOCAL_SALES_TAXES, 'page=' . $_GET['page'] . '&action=insert')];
        $contents[] = ['text' => TEXT_INFO_INSERT_INTRO];
        $contents[] = ['text' => '<br>' . TEXT_INFO_COUNTRY . '<br>' . zen_draw_pull_down_menu('zone_country_id', zen_get_countries(TEXT_ALL_COUNTRIES), '', 'onchange="update_zone(this.form);"')];
        $contents[] = ['text' => '<br>' . TEXT_INFO_COUNTRY_ZONE . '<br>' . zen_draw_pull_down_menu('zone_id', zen_prepare_country_zones_pull_down())];
        $contents[] = ['text' => '<br>' . TEXT_INFO_TAX_RATE . '<br>' . zen_draw_input_field('tax_rate')];
        $contents[] = ['text' => '<br>' . TEXT_INFO_FIELDMATCH . '<br>' . zen_draw_pull_down_menu('tax_fieldmatch', $za_lookup)];
        $contents[] = ['text' => '<br>' . TEXT_INFO_DATAMATCH . '<br>' . zen_draw_textarea_field('tax_datamatch', false, 35, 4)];
        $contents[] = ['text' => '<br>' . TEXT_INFO_RATE_DESCRIPTION . '<br>' . zen_draw_input_field('tax_description')];
        $contents[] = ['text' => '<br />' . TEXT_INFO_TAX_SHIPPING . '<br>' . zen_draw_radio_field('tax_shipping', 'false', true) . ' ' . TEXT_TAX_SHIPPING_FALSE . '<br>' . zen_draw_radio_field('tax_shipping', 'true') . ' ' . TEXT_TAX_SHIPPING_TRUE];
        $contents[] = ['text' => '<br>' . TEXT_INFO_TAX_CLASS_TITLE . '<br>' . zen_tax_classes_pull_down('name="tax_class_id" style="font-size:10px"')];
    ...
    case 'edit':
        ...
        $contents = ['form' => zen_draw_form('local_sales_tax', FILENAME_LOCAL_SALES_TAXES, $link_params . '&action=save')];
        $contents[] = ['text' => TEXT_INFO_EDIT_INTRO];
        $contents[] = ['text' => '<br>' . TEXT_INFO_COUNTRY . '<br>' . zen_draw_pull_down_menu('zone_country_id', zen_get_countries_for_admin_pulldown(), $stInfo->zone_country_id, 'onchange="update_zone(this.form);"')];
        $contents[] = ['text' => '<br>' . TEXT_INFO_COUNTRY_ZONE . '<br>' . zen_draw_pull_down_menu('zone_id', zen_prepare_country_zones_pull_down($stInfo->zone_country_id), $stInfo->zone_id)];
        $contents[] = ['text' => '<br>' . TEXT_INFO_TAX_RATE . '<br>' . zen_draw_input_field('tax_rate', $stInfo->local_tax_rate)];
        $contents[] = ['text' => '<br>' . TEXT_INFO_FIELDMATCH . '<br>' . zen_draw_pull_down_menu('tax_fieldmatch', $za_lookup, $stInfo->local_fieldmatch)];
        $contents[] = ['text' => '<br>' . TEXT_INFO_DATAMATCH . '<br>' . zen_draw_textarea_field('tax_datamatch', false, 35, 4, $stInfo->local_datamatch)];
        $contents[] = ['text' => '<br>' . TEXT_INFO_RATE_DESCRIPTION . '<br>' . zen_draw_input_field('tax_description', $stInfo->local_tax_label)];

        $on_status = ($stInfo->local_tax_shipping === 'true');
        $off_status = !$on_status;

        $contents[] = ['text' => '<br>' . TEXT_INFO_TAX_SHIPPING . '<br>' . zen_draw_radio_field('tax_shipping', 'false', $off_status) . ' ' . TEXT_TAX_SHIPPING_FALSE . '<br>' . zen_draw_radio_field('tax_shipping', 'true', $on_status) . ' ' . TEXT_TAX_SHIPPING_TRUE];
        $contents[] = ['text' => '<br>' . TEXT_INFO_TAX_CLASS_TITLE . '<br>' . zen_tax_classes_pull_down('name="tax_class_id" style="font-size:10px"', $stInfo->local_tax_class_id)];
...
?>
        ...
        <div class="row">
          <table class="table">
            <tr>
              <td><?= $localtax_split->display_count($localtax_query_numrows, MAX_DISPLAY_TAX_RESULTS, $_GET['page'], MAX_DISPLAY_PAGE_LINKS) ?></td>
